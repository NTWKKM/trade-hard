...
- **Timezone format**: Timestamp formatting `fmtT()` is hardcoded to `Asia/Bangkok` (GMT+7). A realtime countdown for the current timeframe updates via a 1-second `setInterval` loop calling `scheduleRender()`.
- **Right Scroll Margin**: The chart view allows panning into the future space by 20%. `clampView()` limits `viewStart` using `maxScroll = bars.length - viewBars * 0.8`. The candle width `bw` is strictly `main.w / viewBars` (instead of `main.w / vis.length`) so candles don't stretch when entering the empty right-side space.
- **Dracula Theme**: The application uses the standard Dracula color palette (`#282a36`, `#50fa7b`, `#ff5555`, `#bd93f9`, etc.) for all tokens in `MarketChart.css` and the canvas config `CFG.CLR` in `MarketChart.tsx`.
