...
// Memoization cache for MA calculations
const maCache = new Map<string, (number | null)[]>();

function getCachedMA(cacheKey: string): (number | null)[] | undefined {
  return maCache.get(cacheKey);
}

function setCachedMA(cacheKey: string, data: (number | null)[]): void {
  maCache.set(cacheKey, data);
  // Clean up old cache entries periodically
  if (maCache.size > 100) {
    const oldestKey = maCache.keys().next().value;
    if (oldestKey) {
      maCache.delete(oldestKey);
    }
  }
}
...
export const rainbowMaIndicator = {
  ...
  calc: (dataList: KLineData[], indicator: Indicator<RainbowData>): RainbowData[] => {
    ...
    const dataHash = dataList.length > 0 ? dataList[0].timestamp + '-' + dataList[dataList.length - 1].timestamp : 'empty';
    const cacheKeyBase = `${dataHash}-${length}-${start}-${maNumber}-${maType}`;

    const maArrays: (number | null)[][] = [];

    for (let c = 0; c < maNumber; c++) {
      const period = length * (start + c);
      const cacheKey = `${cacheKeyBase}-${period}`;
      
      // Try to get from cache first
      let maArray = getCachedMA(cacheKey);
