...
// Memoize symbol and interval options to prevent unnecessary re-renders
const symbolOptions = SYMBOLS.map(s => (
  <option key={s.value} value={s.value}>{s.label}</option>
));

const intervalOptions = INTERVALS.map(t => (
...
export default function MarketChart() {
  ...
  // Memoize the data loading function to prevent recreation on each render
  const loadData = useCallback(async (source: DataSource, symbol: string, timeInterval: string) => {
  ...
  // Debounced data loading effect
  useEffect(() => {
    const debounceTimer = setTimeout(() => {
      loadData(source, symbol, timeInterval);
    }, 400);
    
    return () => clearTimeout(debounceTimer);
  }, [source, symbol, timeInterval, loadData]);
